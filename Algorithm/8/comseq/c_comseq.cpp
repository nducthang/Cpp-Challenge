...
// Cac bien cua tung bai
int n, m, a[102], b[102], c[102], ca[102], cb[102];
...
int cham() 
{
    ...
    for (i=1; i<=kqo; i++) out >> c[i];
    for (i=1; i<=kqo; i++) out >> ca[i];
    for (i=1; i<=kqo; i++) out >> cb[i];
    // Kiem tra ket qua
    for (i=1; i<kqo; i++)
        if ((ca[i]>=ca[i+1]) || (cb[i]>=cb[i+1])) return 11; // Sai chi so
    if ((ca[1]<1) || (ca[kqo]>m) || (cb[1]<1) || (cb[kqo]>n)) return 11; // Sai chi so
    for (i=1; i<=kqo; i++)
        if ((a[ca[i]]!=c[i]) || (b[cb[i]]!=c[i])) return 12; // Khong la day chung
...
int viet()
{
    ...
    for (i=Pmin; i<=Pmax; i++)
    {
        ...
        switch (loi[i])
        {
            ...
            case 11: cout << "Sai chi so"; break;
            case 12: cout << "Khong la day chung"; break;
        }
        cout << '\n';
    }
    cout << "--------------------------" << '\n';
