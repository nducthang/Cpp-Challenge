...
int cham() 
{
    ...
    out >> slx;
    for (i=1; i<=slx; i++) out >> x[i];
    for (i=1; i<=n; i++) dd[i]=0;
    s=0; tt=0;
    for (i=1; i<=slx; i++)
    {
        if ((x[i]<1) || (x[i]>n)) return 11; // Sai so hieu
        if (dd[x[i]]!=0) return 12; // Chon trung
        dd[x[i]]=1;
        s+=a[x[i]];
        tt+=b[x[i]];
    }
    if (s!=kqo) return 13; // Tinh gia tri sai
    if (tt>m) return 14; // Nhieu qua!!!
    return 0;
}    
...
int viet()
{
    ...
    for (i=Pmin; i<=Pmax; i++)
    {
        ...
        switch (loi[i])
        {
            ...
            case 13: cout << "Tinh gia tri sai"; break;
            case 14: cout << "Chon nhieu qua!!!"; break;
        }
        cout << '\n';
    }
    cout << "--------------------------" << '\n';
