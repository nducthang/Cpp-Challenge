...
int cham() 
{
    ...
    for (i=1; i<=n; i++)  {out >> x[i]; if (out.bad()) return 1;}
    long s=0;
    for (i=1; i<=n; i++)
    {
        if ((x[i]<1) || (x[i]>m)) return 11; // Sai so hieu dong
        if ((i<n) && (abs(x[i]-x[i+1])>1)) return 12; // Di sai
        s+=a[x[i]][i];
    }
    if (s!=kqo) return 13; // Tinh tong sai
    return 0;
}    
...
int viet()
{
    ...
    for (i=Pmin; i<=Pmax; i++)
    {
        ...
        switch (loi[i])
        {
            ...
            case 11: cout << "Sai so hieu dong"; break;
            case 12: cout << "Di sai"; break;
