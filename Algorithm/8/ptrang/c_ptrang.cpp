...
int cham() 
{
    ...
    out >> kqo >> slo;
    ans >> kqm;
    if (kqo>kqm) return 10; // wrong answer
    do {out.get(ch);} while (ch!='\n');
    int k=0;
    for (i=1; i<=slo; i++)
    {
        int s=0;
        ...
        {
            int u;
            while (out.peek()==' ')  out.ignore();
            if (out.peek()!='\n')
            {
                out >> u;
                if ((u!=k+1) || (u<1) || (u>n)) return 11; // Sai so hieu
                s+=w[u];
                k=u; u=0;
            }
        }
        if ((s>L) || (L-s>kqo)) return 12; // Chia dong sai
        out.ignore();
    }
    return 0;
}    
...
int viet()
{
    ...
    for (i=Pmin; i<=Pmax; i++)
    {
        ...
        switch (loi[i])
        {
            ...
            case 12: cout << "Chia dong sai"; break;
        }
        cout << '\n';
    }
    cout << "--------------------------" << '\n';
